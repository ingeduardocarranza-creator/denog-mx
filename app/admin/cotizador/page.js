    ...
    <div className="min-h-screen bg-gray-950 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ color: 'white', fontSize: 22, fontWeight: 700 }}>🧮 Cotizador</div>
          <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, marginTop: 2 }}>Calcula el precio de venta de tus productos</div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 14 }}>
            <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Precio en dólares</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 16 }}>$</span>
              <input type="number" value={precioUSD} onChange={e => setPrecioUSD(parseFloat(e.target.value) || 0)}
                style={{ flex: 1, background: 'transparent', border: 'none', color: 'white', fontSize: 22, fontWeight: 700, outline: 'none', width: '100%' }} />
              <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>USD</span>
            </div>
          </div>

          {/* Tipo de cambio */}
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 14 }}>
            <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Tipo de cambio</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 16 }}>$</span>
              <input type="number" value={tipoCambio} onChange={e => setTipoCambio(parseFloat(e.target.value) || 0)}
                style={{ flex: 1, background: 'transparent', border: 'none', color: 'white', fontSize: 22, fontWeight: 700, outline: 'none', width: '100%' }} />
              <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>MXN</span>
            </div>
          </div>

          {/* Tax */}
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 14 }}>
            <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Estado / Tax</div>
              ...
              {taxBtns.map(btn => {
                ...
                return (
                  <button key={btn.id} onClick={() => setTaxTipo(btn.id)}
                    ...
                    <div style={{ color: activo ? colorActivo : 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 600 }}>{btn.label}</div>
                    <div style={{ color: activo ? colorActivo : 'rgba(255,255,255,0.3)', fontSize: 10, opacity: 0.7 }}>{btn.pct}</div>
                  </button>
                )
              })}
            </div>
            {taxTipo === 'denog' && (
              <div style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 10, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <input type="number" value={taxDenog} min="0" max="50" step="0.1"
                  ...
                  style={{ width: 60, background: 'transparent', border: 'none', color: '#f59e0b', fontSize: 18, fontWeight: 700, outline: 'none', textAlign: 'center' }} />
          ...
          {/* Ganancia */}
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 14 }}>
            <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Ganancia</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 16, marginBottom: 12 }}>
          <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>Desglose del precio</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>Precio en USD</span>
            <span style={{ color: 'white', fontSize: 13 }}>{fmt(precioUSD)} USD</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>{getTaxNombre()} ({taxPct}%)</span>
            <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13 }}>+{fmt(taxMonto)} USD</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, paddingBottom: 8, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>Costo total en MXN</span>
            <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13 }}>{fmt(costoMXN)} MXN</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>Tu ganancia ({ganancia}%)</span>
            <span style={{ color: '#10b981', fontSize: 13 }}>+{fmt(ganMXN)} MXN</span>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(193,85,58,0.08)', border: '1px solid rgba(193,85,58,0.2)', borderRadius: 18, padding: 24, textAlign: 'center' }}>
          ...
          <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 11, marginTop: 6, marginBottom: 16 }}>MXN · Redondeado al siguiente $5</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 24 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10, marginBottom: 2 }}>Tu costo</div>
              <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, fontWeight: 600 }}>{fmtR(costoMXN)} MXN</div>
            </div>
            <div style={{ width: 1, background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10, marginBottom: 2 }}>Tu ganancia</div>
              <div style={{ color: '#10b981', fontSize: 14, fontWeight: 600 }}>{fmtR(ganMXN)} MXN</div>
            </div>
            <div style={{ width: 1, background: 'rgba(255,255,255,0.1)' }} />
            <div style={{ textAlign: 'center' }}>
