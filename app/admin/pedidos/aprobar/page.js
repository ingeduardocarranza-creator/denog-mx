...
export default function PorAprobar() {
  ...
  const calcular = p => {
    ...
    const tc = parseFloat(p.tipo_cambio) || 0
  ...
  const guardar = async (p, { pendiente_aprobacion, estado }) => {
    ...
    const body = {
      ...
      tipo_cambio: p.tipo_cambio === '' ? null : parseFloat(p.tipo_cambio),
      impuesto_pct: p.impuesto_pct === '' ? null : parseFloat(p.impuesto_pct),
      costo_mxn,
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                  ...
                  <div>
                    ...
                    <input type="number" step="0.01" style={inp} value={p.tipo_cambio ?? ''} onChange={e => actualizarCampo(p.id, 'tipo_cambio', e.target.value)} />
                  </div>
                  <div>
                    <label style={lbl}>Tax %</label>
                    <input type="number" step="0.1" style={inp} value={p.impuesto_pct ?? ''} onChange={e => actualizarCampo(p.id, 'impuesto_pct', e.target.value)} />
                  </div>
                </div>
