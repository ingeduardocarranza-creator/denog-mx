...
export async function PUT(req) {
  ...
  const { id, cliente_id, entrega_id, descripcion, lugar_compra, cantidad, fecha_compra,
    precio_usd, tipo_cambio, impuesto_pct, costo_mxn, precio_venta, utilidad, notas, estado, vendedor_id, categoria, apartado_fragil, imagen_url } = await req.json()
  ...
  const { data, error } = await supabase
    ...
    .update({ cliente_id, entrega_id, descripcion, lugar_compra, cantidad, fecha_compra,
      precio_usd, tipo_cambio, impuesto_pct, costo_mxn, precio_venta, utilidad, notas, estado, vendedor_id, categoria, apartado_fragil: apartado_fragil || false, imagen_url: imagen_url || null })
    .eq('id', id)
    .select()
    .single()
