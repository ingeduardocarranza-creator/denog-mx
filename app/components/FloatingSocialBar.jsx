...
// Solo se muestra en las pantallas de cara al cliente (Portada, Cliente,
// Mercadito) — en Admin/POS el equipo ya trabaja dentro del sistema y esta
...
export default function FloatingSocialBar() {
  ...
  // carrito pegada abajo cuando hay artículos — subimos la barra social ahí
  // para que no se encimen.
  const elevada = pathname === '/mercadito';
    ...
    <>
      <style>{`
        .denog-fab-wrap {
          position: fixed;
          right: 20px;
          bottom: calc(20px + env(safe-area-inset-bottom) + var(--fab-raise, 0px));
          z-index: 9999;
          display: flex;
        ...
        .denog-fab--whatsapp { background: #25D366; }
        ...
        @media (max-width: 900px) {
          ...
          .denog-fab { width: 46px; height: 46px; }
        }
        @media (max-width: 560px) {
          .denog-fab-wrap { right: 12px; bottom: calc(12px + env(safe-area-inset-bottom) + var(--fab-raise, 0px)); gap: 8px; }
          .denog-fab { width: 42px; height: 42px; }
        }
      `}</style>
