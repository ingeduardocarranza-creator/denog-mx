      ...
      <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800 w-full max-w-sm space-y-4 shadow-xl">
        ...
        <form onSubmit={manejarApertura} className="space-y-4 text-xs">
          <div className="space-y-1.5">
            ...
            <input type="number" value={efectivo} onChange={(e) => setEfectivo(e.target.value)} placeholder="0.00 (Si no hay, pon 0)" className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white tabular-nums text-sm focus:outline-none" />
          </div>
          <button type="submit" disabled={loading} className="w-full bg-[#c1553a] hover:bg-[#9b3f28] sobre-color font-bold py-3.5 rounded-xl uppercase tracking-widest transition-all text-xs">
