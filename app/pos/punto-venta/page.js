...
export default function PuntoDeVenta() {
  ...
  useEffect(() => {
    async function cargarDatosPOS() {
      ...
      setTodasEntregas(en || []);
      const vendedoresLista = (cl || []).filter(c => c.rol === 'admin' || c.rol === 'vendedor')
  ...
  const ajustarCantidadCarrito = (productoId, delta) => {
    setCarritoTienda(prev => prev.map(item => {
      if (item.producto.id === productoId) {
        ...
        if (delta > 0 && nuevaCant > item.producto.stock) return item;
  ...
  const procesarCobroFinal = async () => {
    ...
    carritoTienda.forEach(item => { textoArticulos += `• ${item.producto.nombre} (x${item.cantidad})\n`; });
      ...
      if (modo === 'modo1' && clienteSeleccionado) {
        for (const b of bloquesEntregas) {
          for (const p of b.pedidos) {
            if (productosSeleccionados[p.id]) await supabase.from('pedidos').update({ estado: 'Entregado' }).eq('id', p.id);
          }
        }
      }
          ...
          <div className="max-w-4xl mx-auto grid grid-cols-1 gap-6">
                    ...
                    <div key={idx} className={`border rounded-xl overflow-hidden ${bloque.atrasada ? 'border-red-900 bg-red-950/10' : 'border-gray-800'}`}>
                      ...
                      <div className="divide-y divide-gray-800">
                        ...
                        {anticiposDeEsteBloque.map((anticipo, aIdx) => {
                          const fechaTxt = anticipo.creado_en ? formatearFechaCorta(anticipo.creado_en) : 'Fecha reciente';
                          return (
            ...
            {(modo === 'modo2' || (modo === 'modo1' && clienteSeleccionado)) && (
              <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800 shadow-xl space-y-4">
                ...
                {/* Selector vendedor — solo modo tienda */}
                {modo === 'modo2' && vendedores.length > 1 && (
                  <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {vendedores.map(v => {
                        ...
                        return (
                          <button key={v.id} onClick={() => setVendedorTienda(v)}
                            style={{ flex: 1, minWidth: 80, background: activo ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.03)', border: `1px solid ${activo ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 10, padding: '8px 6px', cursor: 'pointer' }}>
                            <div style={{ width: 28, height: 28, borderRadius: '50%', background: activo ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: activo ? '#818cf8' : 'rgba(255,255,255,0.5)', fontWeight: 700, margin: '0 auto 5px' }}>
                              {iniciales}
                            </div>
                            <div style={{ color: activo ? '#818cf8' : 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: activo ? 600 : 400 }}>{v.nombre.split(' ')[0]}</div>
                            {v.id === colaborador?.id && <div style={{ color: activo ? 'rgba(129,140,248,0.5)' : 'rgba(255,255,255,0.2)', fontSize: 9 }}>En sesión</div>}
                          </button>
                        )
                      })}
                    </div>
                    <div style={{ marginTop: 8, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)', borderRadius: 8, padding: '5px 10px', fontSize: 10, color: 'rgba(245,158,11,0.6)' }}>
                      💡 Cambia el vendedor si el cliente viene referido de otra plataforma
                    </div>
                  </div>
                )}
            ...
            <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800 shadow-xl space-y-4">
              ...
              {modo === 'modo2' && vendedorTienda && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11 }}>Venta asignada a</span>
                  <span style={{ background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.25)', borderRadius: 20, padding: '2px 10px', color: '#818cf8', fontSize: 11, fontWeight: 600 }}>
                    {vendedorTienda.nombre.split(' ')[0]} {vendedorTienda.nombre.split(' ')[1]}
                  </span>
                </div>
              )}
