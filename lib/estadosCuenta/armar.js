...
export function armarPorEntrega({ pedidosDeLaEntrega, todosPedidos, todosPagos, entregas, clientes, entregaId }) {
  ...
  const pedidos = (todosPedidos || []).filter(p =>
    ...
    p.estado !== 'no_llego' &&
    p.estado !== 'Cancelado' &&  // mercancia que no llego y se cancelo: no se cobra
    p.estado !== 'pendiente'     // pausado a mano en la pestana "Pendientes" de
                                  // Empacado -- todavia no es mercancia confirmada,
                                  // por eso tampoco lo muestra el punto de venta
                                  // (ver /api/punto-venta/cliente). Caso real: a
                                  // Cecilia Ramos el estado de cuenta le seguia
                                  // mostrando un articulo pausado que el POS ya no
                                  // ofrecia cobrar (22 sep 2026).
  )
  ...
  const lista = Object.entries(porCliente).map(([id, d]) => {
    ...
    return {
      cliente: { id, nombre: d.nombre || cl?.nombre || '', telefono: cl?.telefono || '', codigoRecoleccion: cl?.codigo_recoleccion || '' },
      grupos
    }
