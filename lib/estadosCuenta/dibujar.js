...
export async function dibujarEstadoCuenta({ cliente, grupos }, entorno) {
  ...
  height += totalH + (fechaLimite ? plazoH : 0) + (mostrarCodigo ? codigoH : 0) + footH
  ...
  if (mostrarCodigo) {
    ...
    izq('MUESTRA ESTE CÓDIGO AL RECOGER', PAD, y + 34 * S, 11, C.suave, true)
    ctx.fillStyle = C.tinta
    ctx.font = `bold ${26 * S}px ${FF}`
    ctx.textAlign = 'left'
    ctx.fillText(cliente.codigoRecoleccion, PAD, y + 74 * S)
    izq('Es tu código personal — no cambia entre entregas', PAD, y + 100 * S, 10.5, C.suave)
    y += codigoH
  }
