...
export const fmtFecha = (f) => {
  ...
  // Se queda con los 10 primeros caracteres: aguanta '2026-08-14',
  // '2026-08-14T22:06:48' y '2026-08-14 22:06:48' por igual. Antes, cualquier
  // formato con hora daba fecha inválida.
  const d = new Date(String(f).slice(0, 10) + 'T12:00:00')
  // Y si aun así no es una fecha, no se imprime nada. Este papel va al cliente:
  // un hueco se entiende, un "NaN undefined NaN" asusta.
...
// Mismo cálculo de "grupo reciente / falta por recoger / fecha límite" que
// usa el dibujo (más abajo, dentro de dibujarEstadoCuenta). Se saca a una
// función aparte para que el mensaje de WhatsApp (lib/whatsapp/enviarDocumentos.js)
// diga exactamente lo mismo que la imagen — nunca una fecha en el texto y
// otra en el papel.
export function resumenParaEnvio(grupos) {
  const grupoReciente = [...(grupos || [])].reverse().find(g => g.entrega)
  const faltaPorRecoger = (grupos || []).some(g => g.pedidos.some(p => p.estado !== 'Entregado'))
  const fechaLimite = faltaPorRecoger ? (grupoReciente?.entrega?.fecha_limite || null) : null
  return {
    saldo: saldoDeGrupos(grupos),
    fechaEntrega: grupoReciente?.entrega?.fecha_entrega || null,
    fechaLimite,
    faltaPorRecoger,
  }
}
...
// La paleta original, con nombre propio: dentro del dibujo `C` queda sombreado
// por la paleta elegida y no puede servir de respaldo de sí mismo.
const PALETA_BASE = C

// La paleta de marca (Cielo), con exactamente las mismas claves que C: así el
// dibujo no cambia ni una línea, solo de dónde saca los colores.
//
// Los nueve colores de marca son pasteles y un pastel no aguanta texto blanco
// encima. El azul marino carga las franjas donde va el dinero; los pasteles se
// quedan en fondos y acentos. `tinta` es el propio marino oscurecido, porque la
// paleta no trae un neutro oscuro y un gris se vería prestado.
export const C_CIELO = {
  tinta:      '#16283b',
  pie:        '#2a415a',
  texto:      '#2a415a',
  suave:      '#4a6480',
  ...
  blanco:     '#ffffff',
  sup2:       '#f7fcff',
  marca:      '#8aacdd',   // azul medio — la franja del cliente
  marcaOsc:   '#466d9b',
  marcaClaro: '#bdecff',
  beigeLinea: '#dcebf5',
  marcaTexto: '#f0f8ff',   // texto secundario sobre el azul medio
  verde:      '#2c7a5d',   // derivado de la menta, oscurecido para poder leerse
  verdeOsc:   '#24634b',
  verdeFondo: '#e9f7f1',
  verdeTexto: '#b8ead6',
}

export const PALETAS_EC = { cielo: C_CIELO }
...
export const MARCOS = {
  ...
  // Cielo: blanco arriba y abajo y marino en el total, igual que el ticket. Los
  // dos papeles que recibe el cliente tienen que verse de la misma casa.
  cielo: {
    fondo: ['#ffffff'], filete: '#466d9b',
    titulo: '#16283b', sub: '#466d9b', texto: '#2a415a', linea: '#dcebf5',
    facebook: null, instagram: null, tiktok: '#16283b', web: '#466d9b',
    totalFondo: '#466d9b', totalFilete: '#33547a',
  },
  degradado: {
...
export function degradadoIG(ctx) {
...
export async function dibujarEstadoCuenta({ cliente, grupos }, entorno) {
  const {
    ...
    // Cielo es el diseño de la casa. Ni la pantalla del panel ni el envío por
    // WhatsApp pasan estos dos, así que el default los mueve a los dos a la vez
    // y siguen coincidiendo — que es la regla de este dibujo desde el principio.
    marco = 'cielo', paleta = 'cielo',
  } = entorno
  const FF = fuente
  const M = MARCOS[marco] || MARCOS.terracota
  // Sombrea la paleta del módulo. Como las claves son idénticas, el resto del
  // dibujo no se entera: cambia el color, no la forma.
  const C = PALETAS_EC[paleta] || PALETA_BASE
  ...
  // El plazo para recoger. Solo tiene sentido si de verdad queda algo por
  // recoger: a quien ya se llevó todo, una fecha límite lo confunde. Misma
  // función que usa el envío por WhatsApp — ver resumenParaEnvio() arriba.
  const { fechaLimite, faltaPorRecoger } = resumenParaEnvio(grupos)
  ...
  const mostrarCodigo = faltaPorRecoger && !!cliente.codigoRecoleccion
  height += totalH + (fechaLimite ? plazoH : 0) + (mostrarCodigo ? codigoH : 0) + horarioH + footH
  ...
  for (const g of grupos) {
    ...
    g.pedidos.forEach((p, i) => {
      ...
      // La descripción y la cantidad van en el tono más oscuro, igual que el
      // importe: los tres son la fila que el cliente lee para comprobar que le
      // están cobrando lo que se llevó. La cantidad iba en el tono más claro de
      // la fila y se perdía.
      izq(recortar(p.descripcion || '', 13, maxW), PAD, yTexto, 13, C.tinta)

      ctx.textAlign = 'center'
      ctx.fillStyle = C.tinta; ctx.font = `${12 * S}px ${FF}`
  ...
  // ================= HASTA CUÁNDO PUEDE RECOGER =====================
  // Va después del total y antes del pie: es lo último que el cliente lee, y
  // es lo único del documento que le pide hacer algo.
  if (fechaLimite) {
    ctx.fillStyle = C.fondo2
    ctx.fillRect(0, y, W, plazoH)
    ctx.fillStyle = C.marcaOsc
    ctx.fillRect(0, y, 5 * S, plazoH)
    izq('PUEDES PASAR POR TU PEDIDO HASTA EL', PAD, y + plazoH / 2 - 11 * S, 10, C.suave, true)
    izq(fmtFecha(fechaLimite), PAD, y + plazoH / 2 + 12 * S, 16, C.tinta, true)
    y += plazoH
  }

  // ================= CÓDIGO PARA RECOGER =============================
  // El código NO es el usuario/contraseña del portal — es uno aparte,
  // permanente, que solo sirve para que en el mostrador confirmen que quien
  // viene por el pedido es a quien le pertenece. Se enseña este renglón (con
  // QR y en texto, por si la cámara no lee bien) para que el cliente lo
  // traiga listo antes de llegar.
  if (mostrarCodigo) {
    ctx.fillStyle = C.sup2
    ctx.fillRect(0, y, W, codigoH)
    ctx.fillStyle = C.linea
    ctx.fillRect(0, y, W, 1 * S)

    const ladoQR = codigoH - 24 * S
    const xQR = W - PAD - ladoQR
    dibujarQR(ctx, cliente.codigoRecoleccion, xQR, y + 12 * S, ladoQR, C.tinta, C.sup2)

    ctx.fillStyle = C.tinta
    ctx.font = `bold ${26 * S}px ${FF}`
    ctx.textAlign = 'left'
    ctx.fillText(cliente.codigoRecoleccion, PAD, y + 66 * S)
    izq('Es tu código personal', PAD, y + 92 * S, 10.5, C.suave)
    y += codigoH
  }

  // ================= HORARIO DE ATENCIÓN ============================
  // Siempre visible, independiente de si falta algo por recoger o no —
  // a diferencia del código de recolección, este dato aplica a todos.
  ctx.fillStyle = C.sup2
  ctx.fillRect(0, y, W, horarioH)
  ctx.fillStyle = C.linea
  ctx.fillRect(0, y, W, 1 * S)
  izq('HORARIO DE ATENCIÓN EN TIENDA', PAD, y + 22 * S, 10, C.suave, true)
  izq('Lun a Vie 10:00 am – 7:00 pm  ·  Sáb 10:00 am – 5:00 pm', PAD, y + 44 * S, 13, C.tinta, true)
  y += horarioH
