...
// Umbral acordado con Eduardo (Fase 5, segregación de funciones): un
// descuento se considera "grande" — necesita que alguien más autorice, sin
// que eso bloquee la venta — si pasa de 20% o de $200, lo que se cumpla
// primero.
export const UMBRAL_DESCUENTO_PCT = 0.20;
export const UMBRAL_DESCUENTO_MONTO = 200;

// Qué tan grande es el descuento TOTAL de la venta (línea + venta,
// combinados) contra el precio de lista, sin ningún descuento. `baseSubtotal`
// es la suma de lo que costarían las líneas sin tocar nada; `total` ya trae
// restados el descuento de línea y el de venta. La diferencia es el
// descuento real que se está dando.
export function evaluarDescuento(cart, descuentoVenta) {
  const totales = calcularTotalesCarrito(cart, descuentoVenta);
  const baseSubtotal = round2(totales.lineas.reduce((s, l) => s + l.baseSubtotal, 0));
  const descuentoTotal = round2(Math.max(0, baseSubtotal - totales.total));
  const descuentoPct = baseSubtotal > 0 ? descuentoTotal / baseSubtotal : 0;
  const esGrande = descuentoTotal > UMBRAL_DESCUENTO_MONTO || descuentoPct > UMBRAL_DESCUENTO_PCT;
  return { baseSubtotal, descuentoTotal, descuentoPct, esGrande };
}
