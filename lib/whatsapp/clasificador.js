...
// decide si merece un pendiente. Ver docs/PENDIENTES.md.
//
// Regla de oro (actualizada 15/ago/2026): ante la duda, SÍ generar pendiente.
// La excepción dura es la venta normal (ver PROMPT_SISTEMA) — eso se calla
// siempre. Este módulo solo avisa — nunca contesta al cliente, nunca aprueba
// pagos, nunca decide nada por sí mismo. Lo usa app/api/whatsapp/webhook por
...
1. "comprobante": el cliente mandó o avisó que mandó un pago — captura de transferencia,
   foto de depósito, PDF del banco (muchos bancos mandan el comprobante como documento PDF,
   no como foto — cuenta igual), o texto tipo "ya te transferí", "te mando el comprobante",
   "ahí va el pago". Una persona tiene que conciliarlo a mano.
...
- Imagen de comprobante de pago (captura de transferencia, depósito, pantalla del banco)
  + "gracias", "mil gracias" o cualquier texto breve de cortesía → "comprobante". La
  imagen manda: un agradecimiento no cancela ni tapa la evidencia de pago. Nunca
  clasifiques solo por el texto cuando hay una imagen adjunta — primero revisa qué
  muestra la imagen.
...
- Cortesías: gracias, saludos, buenos días, emojis sueltos — SOLO cuando no vienen
  acompañadas de una imagen con evidencia de pago. Si el mensaje trae una foto de
  comprobante (transferencia, depósito, captura del banco), esta regla NO aplica sin
  importar qué tan breve o cortés sea el texto: revisa la imagen primero, siempre.
- Preguntas de seguimiento sobre algo ya en curso ("¿ya llegó lo mío?", "¿cuándo me toca?").

REGLA DE PRIORIDAD imagen-sobre-texto: cuando el mensaje trae imagen, el texto nunca
decide solo. Primero evalúa qué muestra la imagen (¿es un comprobante de pago? ¿es un
producto? ¿es de otra tienda?) y usa el texto como apoyo, no como filtro que descarta la
imagen sin mirarla. "gracias" + comprobante = "comprobante", no "ninguna".
...
// entrada:
//  texto        — texto del mensaje, o null si es solo imagen/documento.
//  imagenUrl    — URL pública de la imagen adjunta, si la hay.
//  documentoUrl — URL pública del documento PDF adjunto, si la hay. Muchos
//                 comprobantes de banco llegan como PDF (documento), no como
//                 foto — se manda como bloque "document", no "image": la API
//                 de Claude no puede leer un PDF metido en un bloque de
//                 imagen, aunque el mime type sea válido de otra forma.
...
export async function clasificarMensaje({ texto, imagenUrl, documentoUrl, esReenviada }) {
  if (!texto && !imagenUrl && !documentoUrl) return null

  const contenido = []
  if (imagenUrl) contenido.push({ type: 'image', source: { type: 'url', url: imagenUrl } })
  if (documentoUrl) contenido.push({ type: 'document', source: { type: 'url', url: documentoUrl } })
  ...
  const sinTexto = documentoUrl ? '(sin texto, solo documento PDF adjunto)' : '(sin texto, solo imagen adjunta)'
  contenido.push({
    type: 'text',
