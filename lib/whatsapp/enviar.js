...
// Mensaje de texto simple, sin plantilla ni imagen -- para avisos cortos
// (como el de envío foráneo) cuando la ventana de 24h está abierta.
export async function enviarTextoLibre(waId, texto) {
  const res = await fetch(`${GRAPH}/${phoneNumberId()}/messages`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token()}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
