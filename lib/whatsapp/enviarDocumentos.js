...
export async function enviarTicketDeEntregaPorWhatsapp(supabase, { clienteId, enviadoPor = null, atendio = null }) {
  ...
  // No hay transacción real de la que sacar quién atendió (no hubo cobro
  // nuevo), así que se usa quién está mandando este ticket ahora — es quien
  // de hecho está frente al cliente o al panel en este momento.
  if (atendio) datos.atendio = atendio
  const telefono10 = a10Digitos(datos.cliente.telefono)
  if (!telefono10) {
    return { ok: false, mensaje: 'Ese cliente no tiene teléfono guardado.' }
  }
