...
export async function enviarTicketDeEntregaPorWhatsapp(supabase, { clienteId, enviadoPor = null, atendio = null }) {
  const datos = await cargarTicketDeEntrega(supabase, clienteId)
  // cargarTicketDeEntrega ya trae quién estaba cobrando/entregando en el
  // POS en ese momento (columna pedidos.entregado_por). Solo si esa entrega
  // es de antes de que existiera esa columna (viene null) se usa como
  // respaldo quién está mandando el ticket ahora mismo.
  if (!datos.atendio && atendio) datos.atendio = atendio
  const telefono10 = a10Digitos(datos.cliente.telefono)
  if (!telefono10) {
    return { ok: false, mensaje: 'Ese cliente no tiene teléfono guardado.' }
  }
