...
export async function enviarDomicilioAgendadoPorWhatsapp(supabase, { domicilioId, enviadoPor = null }) {
  ...
  // El anticipo se aplica primero a la mercancía y lo que sobra se aplica
  // al envío -- mismo criterio que getTotalAPagar en
  // app/admin/domicilios/page.js, que resta los anticipos del TOTAL
  // completo (mercancía + envío), no solo de la mercancía. Antes, si el
  // cliente transfería de más justo para cubrir también el envío, ese
  // sobrante se perdía (el Math.max(0, ...) de saldoMercancia lo
  // descartaba) y el aviso seguía mostrando el envío completo como
  // pendiente aunque ya estuviera pagado -- caso real: Rosa Icela
  // Santellano, domicilio del 18 sep 2026.
  const subtotal = Number(dom.subtotal || 0)
  const envioTotal = Number(dom.costo_envio || 0)
  const total = Math.max(0, subtotal + envioTotal - anticipos)
  const saldoMercancia = Math.max(0, subtotal - anticipos)
  const envio = total - saldoMercancia
