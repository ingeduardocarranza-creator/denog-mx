...
export async function encolarMensajeVenta(supabase, msg, telefono) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // Si el mensaje es una RESPUESTA, WhatsApp incluye aquí el id del
      // mensaje citado. Ese es el enlace exacto entre el texto del cliente y
      // su foto — lo único que sobrevive a una ráfaga desordenada.
      responde_a: msg.context?.id || null,
    })
    .select('*')
    .maybeSingle()
...
async function ventaAbiertaPrevia(supabase, telefono) {
  ...
  const { data: p } = await supabase
    .from('pedidos')
    .select('id, cliente_id, precio_usd, precio_venta, descripcion, notas, estado, pendiente_aprobacion, categoria, imagen_url')
...
// ─── Camino especial: TIENDA ────────────────────────────────────────────────
// Cuando el texto de la venta trae como "cliente" la palabra TIENDA (y solo
// esa palabra — ver normalizarNombre), no es una venta de cliente: es abasto
// para el propio Catálogo. El monto que normalmente sería el costo en dólares
// que pagó el cliente se guarda igual como costo en dólares, pero el destino
// cambia por completo: nunca se convierte en pedido. Se manda a la misma
// bandeja "Por aprobar" que ya usa Catálogo para lo que agregan los
// colaboradores (ver db/migrations/2026-07-11_catalogo_aprobacion.sql), para
// que ahí se le agregue precio de venta y lo demás.
function esTienda(nombre) {
  return normalizarNombre(nombre) === 'tienda'
}
...
// La foto de una venta vive en el bucket privado whatsapp-media. Catálogo
// espera una URL pública fija —así funciona ya cualquier otra foto de
// producto—, así que aquí se copia al bucket público `productos`, igual que
// si Eduardo la hubiera subido a mano desde el formulario de Catálogo.
async function copiarFotoAProductos(supabase, rutaWhatsapp) {
  if (!rutaWhatsapp) return null
  try {
    const { data: archivo, error: errDescarga } = await supabase.storage
      .from('whatsapp-media').download(rutaWhatsapp)
    if (errDescarga) throw errDescarga
    const buffer = Buffer.from(await archivo.arrayBuffer())
    const destino = `whatsapp-tienda/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${extensionDeRuta(rutaWhatsapp)}`
    const { error: errSubida } = await supabase.storage
      .from('productos').upload(destino, buffer, { contentType: archivo.type || 'image/jpeg' })
    if (errSubida) throw errSubida
    const { data: pub } = supabase.storage.from('productos').getPublicUrl(destino)
    return pub.publicUrl
  } catch (err) {
    console.error('[bandeja] no se pudo copiar la foto de TIENDA a productos:', err?.message)
    return null
  }
}

// Convierte lo que ya se sabía de la foto (descripción y categoría que puso
// la IA) más el costo en dólares que traía el texto TIENDA, en un artículo
// pendiente de Catálogo — y descarta el pedido que se había abierto para la
// foto, porque nunca fue una venta.
async function moverACatalogoPendiente(supabase, pedido, { costoUsd, cantidad }) {
  const imagenPublica = await copiarFotoAProductos(supabase, pedido.imagen_url)
  const nombre = (pedido.descripcion && pedido.descripcion !== SIN_DESCRIPCION)
    ? pedido.descripcion.replace(/\s*—\s*talla\s+\S+$/i, '').trim()
    : 'Producto de WhatsApp (TIENDA) — revisar foto'

  const { error: errInsert } = await supabase.from('productos_tienda').insert({
    nombre,
  ...
  if (errInsert) {
    console.error('[bandeja] TIENDA: no se pudo crear el pendiente de Catálogo:', errInsert.message)
    return
  }

  await supabase.from('pedidos').update({
    estado: 'descartado',
    notas: [pedido.notas, 'Enviado a Catálogo > Por aprobar (TIENDA)'].filter(Boolean).join(' · '),
  }).eq('id', pedido.id)
}
...
// ─── Enlace explícito: el texto es una respuesta a la foto ─────────────────
// Vale más que cualquier regla de orden. Devuelve:
//   'aplicado'     — se pegó al pedido de la foto citada
//   'esperar'      — la foto citada existe pero todavía no se arma; se reintenta
//                    al final de esta pasada, y si no, en la siguiente
//   'sin-objetivo' — la cita no apunta a una foto de venta utilizable; que lo
//                    resuelva la regla de orden
async function aplicarPorRespuesta(supabase, fila, d, clientes) {
  const { data: objetivo } = await supabase
    .from('whatsapp_ventas_bandeja')
    .select('mensaje_wa_id, clase, estado, pedido_id')
    .eq('mensaje_wa_id', fila.responde_a)
    .maybeSingle()

  if (!objetivo || objetivo.clase !== 'media') return 'sin-objetivo'
  if (objetivo.estado !== 'armado' || !objetivo.pedido_id) return 'esperar'

  const { data: p } = await supabase
    .from('pedidos')
    .select('id, cliente_id, precio_usd, descripcion, notas, estado, pendiente_aprobacion, categoria, imagen_url')
    .eq('id', objetivo.pedido_id)
    .maybeSingle()
  if (!p || !p.pendiente_aprobacion || p.estado === 'descartado') return 'sin-objetivo'
  // La foto ya tiene cliente (alguien se le adelantó, o son dos respuestas a la
  // misma foto). No se pisa: que la regla de orden lo deje como tarjeta suelta,
  // a la vista.
  if (p.cliente_id || p.precio_usd != null) return 'sin-objetivo'

  if (esTienda(d.cliente_nombre)) {
    await moverACatalogoPendiente(supabase, p, { costoUsd: d.precio_usd, cantidad: d.cantidad })
    await marcarArmada(supabase, fila.mensaje_wa_id, p.id)
    return 'aplicado'
  }
  ...
  await supabase.from('pedidos').update({
    ...
    descripcion: conTalla(p.descripcion || DESCRIPCION_PENDIENTE, d.talla),
...
async function insertarPedido(supabase, campos) {
  const { data, error } = await supabase.from('pedidos').insert({
    ...
    creado_via: 'whatsapp',
    ...campos,
  }).select('id, cliente_id, precio_usd, precio_venta, descripcion, notas, categoria, imagen_url').maybeSingle()
  if (error) { console.error('[bandeja] no se pudo crear el borrador:', error.message); return null }
  return data
}

// El texto solo se puede adelantar a su foto cuando los dos salen casi al mismo
// instante: la inversión la causa que Meta tarde ~1 s más en procesar una
// imagen que un texto suelto. Con el trabajo en vivo —mandar la foto, verla
// llegar, y ya entonces contestarla— pasan segundos de por medio y esto no se
// dispara nunca; queda como red para el caso raro.
//
// La ventana es corta a propósito. Con una ventana ancha, un texto cuya foto
// falló se agarraría de la foto de la venta SIGUIENTE en vez de quedarse como
// tarjeta suelta a la vista, y una tarjeta suelta es mucho mejor que una venta
// mal armada que nadie va a notar.
const VENTANA_ADELANTO_MS = 15_000

function pedidoDeFoto(fila) {
  const d = fila.datos || {}
  return {
    descripcion: d.descripcion || SIN_DESCRIPCION,
    categoria: d.categoria || null,
    imagen_url: fila.imagen_path || null,
    precio_venta: d.precio_venta ?? null,
    // Lo que venía escrito junto a la foto (talla, "6 pack", la pregunta de la
    // clienta). No es el título —ese lo hace la IA— pero tampoco se tira.
...
export async function armarVentas(supabase) {
  ...
  try {
    ...
    // Fotos a las que alguien respondió en esta tanda: su cliente va a llegar
    // por el enlace explícito, así que no se dejan abiertas para que un texto
    // suelto se las quede por orden.
    const conRespuesta = new Set((filas || []).filter(x => x.responde_a).map(x => x.responde_a))
    const diferidos = []
    ...
    const lista = filas || []
    const consumidos = new Set() // fotos ya armadas por adelanto

    for (let i = 0; i < lista.length; i++) {
      const f = lista[i]
      if (consumidos.has(f.mensaje_wa_id)) continue
      ...
      if (f.clase === 'media') {
        const nuevo = await insertarPedido(supabase, pedidoDeFoto(f))
        if (!nuevo) continue
        await marcarArmada(supabase, f.mensaje_wa_id, nuevo.id)
        // Si ya hay una respuesta apuntando a esta foto, no se abre para el
        // emparejamiento por orden: su cliente llega por el enlace.
        abiertas.set(f.telefono, conRespuesta.has(f.mensaje_wa_id) ? null : nuevo)
        armados++
        continue
      }
      ...
      // Texto de cliente + costo. Si vino como respuesta a una foto, ese
      // enlace manda sobre cualquier regla de orden.
      if (f.responde_a) {
        const r = await aplicarPorRespuesta(supabase, f, d, clientes)
        if (r === 'aplicado') { armados++; continue }
        if (r === 'esperar') { diferidos.push(f); continue }
        // 'sin-objetivo': sigue con la regla de orden, abajo.
      }
      ...
      if (abierta && !abierta.cliente_id && abierta.precio_usd == null && esTienda(d.cliente_nombre)) {
        await moverACatalogoPendiente(supabase, abierta, { costoUsd: d.precio_usd, cantidad: d.cantidad })
        await marcarArmada(supabase, f.mensaje_wa_id, abierta.id)
        abiertas.set(f.telefono, null)
      } else if (abierta && !abierta.cliente_id && abierta.precio_usd == null) {
        await supabase.from('pedidos').update({
          cliente_id,
          precio_usd: d.precio_usd ?? null,
          cantidad: d.cantidad || 1,
        ...
        // No hay foto abierta. Antes de darlo por huérfano: en una ráfaga el
        // texto llega ANTES que su propia foto, porque Meta tarda más en
        // procesar una imagen que un texto suelto. Así que se mira si el
        // siguiente mensaje de ESTA MISMA fila es una foto que salió en el
        // mismo instante — si sí, es la suya, y se arma adelantada.
        const siguiente = lista.slice(i + 1).find(x =>
          x.telefono === f.telefono && x.estado === 'listo' && !consumidos.has(x.mensaje_wa_id))
        const esSuFoto = siguiente
          && siguiente.clase === 'media'
          && !conRespuesta.has(siguiente.mensaje_wa_id)
          && Math.abs(new Date(siguiente.recibido_en) - new Date(f.recibido_en)) <= VENTANA_ADELANTO_MS

        if (esSuFoto) {
          const camposFoto = pedidoDeFoto(siguiente)
          const nuevo = await insertarPedido(supabase, camposFoto)
          if (nuevo) {
            await marcarArmada(supabase, siguiente.mensaje_wa_id, nuevo.id)
            consumidos.add(siguiente.mensaje_wa_id)
            if (esTienda(d.cliente_nombre)) {
              await moverACatalogoPendiente(supabase, {
                id: nuevo.id,
                descripcion: camposFoto.descripcion,
                categoria: camposFoto.categoria,
                imagen_url: camposFoto.imagen_url,
                notas: camposFoto.notas,
              }, { costoUsd: d.precio_usd, cantidad: d.cantidad })
            } else {
              await supabase.from('pedidos').update({
                cliente_id,
                precio_usd: d.precio_usd ?? null,
                cantidad: d.cantidad || 1,
                descripcion: conTalla(nuevo.descripcion || DESCRIPCION_PENDIENTE, d.talla),
                notas: [nuevo.notas, nota].filter(Boolean).join(' · ') || null,
              }).eq('id', nuevo.id)
            }
            await marcarArmada(supabase, f.mensaje_wa_id, nuevo.id)
            abiertas.set(f.telefono, null) // pareja completa
            armados += 2
            continue
          }
        }
    ...
    // Las respuestas cuya foto todavía no existía cuando les tocó turno. En una
    // ráfaga el texto llega ANTES que su foto, así que este caso es la norma,
    // no la excepción: para cuando termina el recorrido, la foto ya se armó.
    for (const f of diferidos) {
      if ((await aplicarPorRespuesta(supabase, f, f.datos || {}, clientes)) === 'aplicado') armados++
      // Si sigue sin resolverse, se queda en 'listo' y se reintenta en la
      // siguiente pasada — sin bloquear a nadie, porque el enlace es explícito.
    }
