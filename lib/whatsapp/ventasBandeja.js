...
export async function enriquecerFila(supabase, fila) {
  try {
    ...
    if (fila.clase === 'media') {
      ...
      // El precio de venta viene en el caption ("$335", o "$65 Pasta Sauce…").
      const { precio, descripcion: descDelCaption } = extraerPrecioYDescDeCaption(fila.texto)
      ...
      datos = {
        descripcion: descDelCaption || deLaFoto.descripcion || (fila.tipo_wa === 'image' ? SIN_DESCRIPCION : `(${fila.tipo_wa} — revisar el adjunto)`),
...
// armada, si su pedido sigue sin cliente. Así una foto que llegó al final de
// una tanda se puede cerrar con el texto que llegue en la siguiente, sin
// guardar estado aparte.
async function ventaAbiertaPrevia(supabase) {
  ...
  const { data: p } = await supabase
    .from('pedidos')
    .select('id, cliente_id, precio_usd, precio_venta, descripcion, estado, pendiente_aprobacion')
...
async function insertarPedido(supabase, campos) {
  const { data, error } = await supabase.from('pedidos').insert({
    ...
    creado_via: 'whatsapp',
    ...campos,
  }).select('id, cliente_id, precio_usd, precio_venta, descripcion').maybeSingle()
...
export async function armarVentas(supabase) {
  ...
  try {
    ...
    let abierta = await ventaAbiertaPrevia(supabase)
    let armados = 0

    for (const f of filas || []) {
      // Un mensaje a medio enriquecer detiene el recorrido: lo que viene
      // después no se puede colocar sin saber qué era éste. Se armará en la
      // siguiente pasada, cuando ya esté listo.
      if (f.estado === 'pendiente') break
      ...
      if (f.clase === 'media') {
        ...
        abierta = nuevo // esta foto queda esperando su texto de cliente
        armados++
        continue
      }
      ...
      if (abierta && !abierta.cliente_id && abierta.precio_usd == null) {
        ...
        await marcarArmada(supabase, f.mensaje_wa_id, abierta.id)
        abierta = null // cerrada: el siguiente texto ya no la puede tocar
      } else {
