...
// pegado a una descripción de catálogo (ej. reenviar un catálogo de
// proveedor: "$215 Jordan's Skinny Syrups Naturally Sweetened Pumpkin
// Spice Syrup - 12.7oz"). Antes esto no se leía en absoluto porque
// esSoloPrecioMXN() exige que el caption sea nada más el número — se
// perdía el precio de venta y se dejaba "falta venta MXN" aunque sí venía.
// Si el caption empieza con "$<número> <texto>", se toma el número como
// precio y el resto como descripción — que además suele ser más específica
// que lo que adivina la IA de visión, así que se usa en vez de esa.
function extraerPrecioYDescDeCaption(caption) {
  ...
  const m = caption.trim().match(/^\$\s*(\d+(?:\.\d{1,2})?)\s+(.+)$/s)
  if (!m) return { precio: null, descripcion: null }
  return { precio: Number(m[1]), descripcion: m[2].trim().replace(/\s+/g, ' ') }
}
...
export async function procesarMensajeVentaFoto(supabase, { pathImagen, imagenUrlFirmada, caption }) {
  ...
  // Si el caption de la foto es solo un número, o es "$<precio> <descripción
  // de catálogo>", ahí viene el precio de venta en pesos — pasa directo, sin
  // IA (ver nota de flujo real arriba). Si no trae nada o trae otra cosa, se
  // deja para el mensaje de precio aparte.
  const { precio: precioDelCaption, descripcion: descripcionDelCaption } = extraerPrecioYDescDeCaption(caption)
  const descripcionFoto = descripcionDelCaption || datos.descripcion || '(sin descripción — revisar foto)'
