...
//
// Segunda vuelta (30/ago/2026, misma tarde): un borrador YA aprobado volvió
// a recibir un texto nuevo y le duplicó la talla. La búsqueda de "huérfano
// de foto" solo pedía tener imagen — no excluía un borrador que ya tenía
// cliente_id/precio_usd de una ronda anterior. Corregido abajo exigiendo
// cliente_id y precio_usd nulos (y estado != descartado) en ambas búsquedas.
...
export async function procesarMensajeVentaTexto(supabase, { texto }) {
  ...
  // tener imagen Y no tener aún cliente_id — un borrador que YA se completó
  // en una ronda anterior (aprobado, editado o simplemente ya emparejado)
  // nunca debe volver a tomarse como huérfano. Sin el filtro de cliente_id,
  // un texto nuevo podía "pegarse" a un borrador viejo ya aprobado en lugar
  // de a la foto recién llegada — eso fue exactamente lo que pasó en la
  // prueba del 30/ago: una venta ya aprobada terminó con dos tallas
  // encimadas porque un segundo mensaje de texto la encontró como huérfana.
